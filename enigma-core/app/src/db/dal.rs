...
mod test {
    ...
    fn test_force_update_no_key_success() {
        ...
        let val = b"Enigma";
        db.create(&DeltaKey { contract_id, key_type }, &val[..]).unwrap();
        let accepted_val = db.read(&DeltaKey { contract_id, key_type }).unwrap();
        assert_eq!(accepted_val, val);

        // update a different delta
        let key_type = Stype::Delta(2);
        let val_update = b"enigma_rocks";
        db.force_update(&DeltaKey { contract_id, key_type }, &val_update[..]).unwrap();
        let accepted_val = db.read(&DeltaKey { contract_id, key_type }).unwrap();
        assert_eq!(accepted_val, val_update);
    }
