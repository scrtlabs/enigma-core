...
extern { fn ocall_new_delta(retval: *mut i8, enc_delta: *const u8, delta_len: usize, delta_hash: &[u8; 32], _delta_index: *const u32) -> sgx_status_t; }
...
pub fn save_delta(enc: &EncryptedPatch) -> Result<(), EnclaveError> {
    let mut res_int: i8 = -1;
    let h: [u8; 32] = [181, 71, 210, 141, 65, 214, 242, 119, 127, 212, 100, 4, 19, 131, 252, 56, 173, 224, 167, 158, 196, 65, 19, 33, 251, 198, 129, 58, 247, 127, 88, 162];
    let i: u32 = 57;
    let res_status: sgx_status_t = unsafe {
