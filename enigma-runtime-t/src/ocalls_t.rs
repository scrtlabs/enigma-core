...
extern "C" {
    ...
    fn ocall_new_delta(retval: *mut EnclaveReturn, db_ptr: *const RawPointer,
                       ...
                       contract_id: &ContractAddress, _delta_index: *const u32) -> sgx_status_t;
...
pub fn save_state(db_ptr: *const RawPointer, enc: &EncryptedContractState<u8>) -> Result<(), EnclaveError> {
    let mut retval = EnclaveReturn::default();
    let res_status: sgx_status_t = unsafe { ocall_update_state(&mut retval, db_ptr, &enc.contract_id, enc.json.as_c_ptr(), enc.json.len()) };
    match retval {
...
pub fn save_delta(db_ptr: *const RawPointer, enc: &EncryptedPatch) -> Result<(), EnclaveError> {
    ...
    let res_status =
        unsafe { ocall_new_delta(&mut res, db_ptr, enc.data.as_c_ptr(), enc.data.len(), &enc.contract_id, &enc.index as *const u32) };
...
pub fn get_state(db_ptr: *const RawPointer, addr: ContractAddress) -> Result<EncryptedContractState<u8>, EnclaveError> {
    let mut retval = EnclaveReturn::default();
    let mut state_len = 0usize;
    let status = unsafe { ocall_get_state_size(&mut retval, db_ptr, &addr, &mut state_len) };
    ...
    let status = unsafe { ocall_get_state(&mut retval, db_ptr, &addr, state.as_mut_ptr(), state_len) };
    if retval != EnclaveReturn::Success || status != sgx_status_t::SGX_SUCCESS {
        return Err(EnclaveError::OcallError {
...
pub fn get_deltas(db_ptr: *const RawPointer, addr: ContractAddress, start: u32, end: u32) -> Result<Vec<EncryptedPatch>, EnclaveError> {
    let len = (end - start) as usize;
    let mut deltas_buff = vec![0usize; len];
    let mut retval = EnclaveReturn::default();
    let status =
        unsafe { ocall_get_deltas_sizes(&mut retval, db_ptr, &addr, &start as *const u32, &end as *const u32, deltas_buff.as_mut_ptr(), len) };
    if retval != EnclaveReturn::Success || status != sgx_status_t::SGX_SUCCESS {
        return Err(EnclaveError::OcallError {
    ...
    let status =
        unsafe { ocall_get_deltas(&mut retval, db_ptr, &addr, &start as *const u32, &end as *const u32, deltas.as_mut_ptr(), deltas.len()) };
    if retval != EnclaveReturn::Success || status != sgx_status_t::SGX_SUCCESS {
        return Err(EnclaveError::OcallError {
            command: "get_deltas".to_string(),
            err: format!("Error with SGX, retval: {}, status: {:?}", retval, status),
...
pub mod tests {
    ...
    pub fn test_me(db_ptr: *const RawPointer) {
    ...
    pub fn test_get_deltas(db_ptr: *const RawPointer) {
        let contract_id = b"test_get_deltas".sha256();
        let (start, end) = (1, 7);
        let deltas = save_deltas(db_ptr, start, end, &contract_id);
        let res = get_deltas(db_ptr, contract_id, start, end).unwrap();
        assert_eq!(res, deltas);
    }

    pub fn test_get_deltas_more(db_ptr: *const RawPointer) {
        let contract_id = b"test_get_deltas_more".sha256();
        let (start, end) = (1, 15);
        let deltas = save_deltas(db_ptr, start, end, &contract_id);
        let res = get_deltas(db_ptr, contract_id, start, end + 3).unwrap();
        assert_eq!(res, deltas);
    }

    pub fn test_state(db_ptr: *const RawPointer) {
        ...
        save_state(db_ptr, &enc).unwrap();
        let ret = get_state(db_ptr, contract_id).unwrap();
        assert_eq!(enc, ret);
    }

    fn save_deltas(db_ptr: *const RawPointer, start: u32, end: u32, contract_id: &ContractAddress) -> Vec<EncryptedPatch> {
        let mut deltas = Vec::new();
        for i in start..end {
            let mut delta_data = b"data".sha256().to_vec();
            delta_data.push(i as u8);
            let delta = EncryptedPatch { data: delta_data, contract_id: *contract_id, index: i };
