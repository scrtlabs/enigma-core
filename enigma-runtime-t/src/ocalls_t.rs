...
pub fn save_state(enc: &EncryptedContractState<u8>) -> Result<(), EnclaveError> {
    ...
    let res_status: sgx_status_t = unsafe { ocall_update_state(&mut retval, &enc.contract_id, enc.json.as_c_ptr(), enc.json.len()) };
    match retval {
...
pub fn save_delta(enc: &EncryptedPatch) -> Result<(), EnclaveError> {
    ...
    let res_status =
        unsafe { ocall_new_delta(&mut res, enc.data.as_c_ptr(), enc.data.len(), &enc.contract_id, &enc.index as *const u32) };
...
pub fn get_deltas(addr: ContractAddress, start: u32, end: u32) -> Result<Vec<EncryptedPatch>, EnclaveError> {
    ...
    let mut retval = EnclaveReturn::default();
    let status =
        unsafe { ocall_get_deltas_sizes(&mut retval, &addr, &start as *const u32, &end as *const u32, deltas_buff.as_mut_ptr(), len) };
    if retval != EnclaveReturn::Success || status != sgx_status_t::SGX_SUCCESS {
        return Err(EnclaveError::OcallError {
    ...
    let status =
        unsafe { ocall_get_deltas(&mut retval, &addr, &start as *const u32, &end as *const u32, deltas.as_mut_ptr(), deltas.len()) };
    if retval != EnclaveReturn::Success || status != sgx_status_t::SGX_SUCCESS {
        return Err(EnclaveError::OcallError {
            command: "get_deltas".to_string(),
            err: format!("Error with SGX, retval: {}, status: {:?}", retval, status),
