...
impl PrincipalHttpServer {
    ...
    pub fn start(&self) {
        ...
        ///
        io.add_method("get_state_keys", move |params: Params| {
            let request = params.parse::<StateKeyRequest>()?;
            println!("Got get_state_keys request: {:?}", request);
            let response = get_enc_state_keys(
                eid.load(Ordering::SeqCst), request).unwrap();
            let response_data = serde_json::to_string_pretty(&response).unwrap();
            Ok(Value::String(response_data))
        });
