...
use std::{convert::*, fs::File, string::ToString};
...
fn get_contract_methods(input: syn::Item) -> Vec<syn::TraitItemMethod> {
    ...
    match input {
        syn::Item::Trait(input) => {
            v = input.items;
            v.iter().filter_map(|item| match item {
...
fn get_arg_types(method: &syn::TraitItemMethod) -> Vec<proc_macro2::TokenStream> {
    method
        ...
        .filter_map(|arg| match arg {
            ...
            syn::FnArg::Ignored(type_only) => Some(quote! {#type_only}),
            _ => None,
        })
        .collect()
}
...
fn generate_dispatch(input: syn::Item) -> proc_macro2::TokenStream {
    let it: Vec<proc_macro2::TokenStream> = get_contract_methods(input)
        .iter()
        .filter_map(|item| {
            ...
            let return_params_number_literal =
                syn::Lit::Int(syn::LitInt::new(return_params_number as u64, syn::IntSuffix::Usize, proc_macro2::Span::call_site()));
...
impl Write for ParamType {
    ...
    fn error(&self) -> String { format!("The type {} is not supported", self.to_string()) }
}
...
fn generate_eth_functions(contract: &Contract) -> Result<Vec<proc_macro2::TokenStream>, EngWasmError> {
    ...
    for function in &contract.functions {
        ...
        let args_types = function.1.inputs.iter().map(|input| input.kind.clone()).collect();
    ...
    let result: Vec<proc_macro2::TokenStream> = functions
        ...
        .map(|function| {
            ...
            let args_number =
                syn::Lit::Int(syn::LitInt::new(args_ast_types.len() as u64, syn::IntSuffix::Usize, proc_macro2::Span::call_site()));
