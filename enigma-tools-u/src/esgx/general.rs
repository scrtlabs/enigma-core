...
use sgx_types::{sgx_misc_attribute_t,sgx_launch_token_t, sgx_attributes_t, SgxResult};
...
pub fn init_enclave(token_path: &path::PathBuf, use_token: bool, enclave_location: &str) -> SgxResult<(SgxEnclave, Option<sgx_launch_token_t>)> {
    ...
    let enclave = SgxEnclave::create(enclave_location,
                                     debug,
                                     &mut launch_token,
                                     &mut launch_token_updated,
                                     &mut misc_attr)?;

    if launch_token_updated != 0 { return Ok( (enclave, Some(launch_token)) ) }
    Ok( (enclave, None) )
}